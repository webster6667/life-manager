import { NodeLabel } from '@easy-diagram/components/node/NodeLabel'
import { Port } from '@easy-diagram/components/port/Port'
import { IUseStylingOptions, useStyling } from '@easy-diagram/hooks/useStyling'
import { observer } from 'mobx-react-lite'
import React, { useMemo } from 'react'
import { INodeVisualComponentProps } from '@easy-diagram/states/nodesSettings'
import { NodeState } from '@easy-diagram/states/nodeState'
import { IPortFinalState } from '@easy-diagram/states/portState'
import { IComponentDefinition, VisualComponent } from '@easy-diagram/states/visualComponentState'

const NodeDefault: React.FC<INodeVisualComponentProps<INodeDefaultSettings>> = observer(
  ({ entity, settings }) => {
...
export type INodeDefaultSettingsWithoutPorts = Omit<INodeDefaultSettings, 'ports'>

export const createInputOutputHorizontalNode = (settings?: INodeDefaultSettingsWithoutPorts) =>
  createNode({
    ...settings,
    ports: [
...
export const createInputOutputVerticalNode = (settings?: INodeDefaultSettingsWithoutPorts) =>
  createNode({
    ...settings,
    ports: [
...
export const createInputHorizontalNode = (settings?: INodeDefaultSettingsWithoutPorts) =>
  createNode({
    ...settings,
    ports: [{ id: 'input', type: 'input', position: 'left-center' }]
  })

export const createInputVerticalNode = (settings?: INodeDefaultSettingsWithoutPorts) =>
  createNode({
    ...settings,
    ports: [{ id: 'input', type: 'input', position: 'top-center' }]
  })

export const createOutputHorizontalNode = (settings?: INodeDefaultSettingsWithoutPorts) =>
  createNode({
    ...settings,
    ports: [{ id: 'output', type: 'output', position: 'right-center' }]
  })

export const createOutputVerticalNode = (settings?: INodeDefaultSettingsWithoutPorts) =>
  createNode({
    ...settings,

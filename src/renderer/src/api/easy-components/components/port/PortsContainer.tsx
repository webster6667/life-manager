...
import { VisualComponent } from '@easy-diagram/states/visualComponentState'
import { positionValues, Position } from '@easy-diagram/utils/position'
import { IPortState } from '@easy-diagram/states/portState'
...
const PortsContainer: React.FC<IPortsContainerProps & IPortsContainerSettings> = observer(
  ({ position, ports, style, gapBetweenPorts, offsetFromOriginPosition }) => {
    ...
    return (
      ...
      >
        {portsModified && portsModified.map((port) => <Port {...port} key={port.id} />)}
      </div>
    )
  }
)
...
export function createPortsContainer(
  ...
  return (props: IPortsContainerProps) => (
    <PortsContainer {...{ ...defaultPortsContainerSettings, ...settings }} {...props} />
  )
}

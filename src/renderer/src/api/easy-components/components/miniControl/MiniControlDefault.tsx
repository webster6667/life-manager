import { cloneSelectedNodesCommand } from '@easy-diagram/commands/clone'
import { removeSelectedCommand } from '@easy-diagram/commands/remove'
import {
  CopyIcon,
  FilterCenterFocusIcon,
  RubbishBinIcon,
  UnlockIcon
} from '@easy-diagram/components/Icons'
import { LockIcon } from '@easy-diagram/index'
import { observer } from 'mobx-react-lite'
import React, { useMemo } from 'react'
import type { IMiniControlComponentProps } from '@easy-diagram/states/diagramSettings'
import { IComponentDefinition } from '@easy-diagram/states/visualComponentState'
import { combineArrays } from '@easy-diagram/utils/common'
import { CornerPosition } from '@easy-diagram/utils/position'

const MiniControlDefault: React.FC<IMiniControlComponentProps<IMiniControlDefaultSettings>> =
  observer(
    ({
      rootStore: { commandExecutor, diagramState, diagramSettings, selectionState },
      settings
    }) => {
      ...
      const className = useMemo(
        () =>
          combineArrays(['react_fast_diagram_MiniControl_Default'], finalSettings?.classes).join(
            ' '
          ),
        [finalSettings]
      )
      ...
      return (
        <div className={className} style={style}>
          {finalSettings.buttons.deleteSelection && selectionState.selectedItems.length > 0 && (
            <Button
          ...
          {finalSettings.buttons.cloneSelectedNodes && selectionState.selectedNodes.length > 0 && (
            <Button
              size={finalSettings.buttonsSize}
          ...
          {finalSettings.buttons.zoomIn && (
            <Button size={finalSettings.buttonsSize} onClick={diagramState.zoomIn} children="+" />
          )}
          {finalSettings.buttons.zoomOut && (
            <Button size={finalSettings.buttonsSize} onClick={diagramState.zoomOut} children="-" />
          )}
          {finalSettings.buttons.zoomToFit && (
            <Button size={finalSettings.buttonsSize} onClick={diagramState.zoomToFit}>
              <FilterCenterFocusIcon />
            </Button>
          )}
            ...
            <Button
              ...
              onClick={() => {
                if (diagramSettings.userInteraction.arePointerInteractionsDisabled)
                  diagramSettings.userInteraction.enableAllPointerInteractions(true)
                else diagramSettings.userInteraction.disableAllPointerInteractions(true)
              }}
            >
...
): IComponentDefinition<IMiniControlComponentProps, IMiniControlDefaultSettings> => {
  const finalSettings = {
    ...defaultSettings,
    ...(settings ? settings : {})
  }

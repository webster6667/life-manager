...
import { subtractPoints, multiplyPoint, addPoints } from '@easy-diagram/utils/point'
import { LinkPointEndpointState } from '@easy-diagram/states/linkPointEndpointState'
import { LinkPortEndpointState } from '@easy-diagram/states/linkPortEndpointState'
import { ILinkInteractionState, ILinkPath, createLinkPath } from '@easy-diagram/states/linkState'
...
export class LinkCreationState implements ILinkInteractionState {
  ...
  startLinking = (portState: PortState, pointOnPort: Point): boolean => {
    ...
    if (sourcePoint && portSize) {
      ...
      const topLeftCornerPoint = subtractPoints(sourcePoint, multiplyPoint(portSize, 0.5))

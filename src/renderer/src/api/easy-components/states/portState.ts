...
import { HtmlElementRefState } from '@easy-diagram/states/htmlElementRefState'
...
import { COMPONENT_DEFAULT_TYPE } from '@easy-diagram/states/visualComponents'
import { isBoolean, deepCopy } from '@easy-diagram/utils/common'
import { PortPosition } from '@easy-diagram/hooks/useRelativePositionStyles'
...
export class PortState {
  ...
  constructor(rootStore: RootStore, node: NodeState, id: string, state?: IPortStateWithoutIds) {
    this._id = id
    this._node = node
  ...
  get nodeComponentPortState(): IPortFinalState | undefined {
    return this.node.componentDefinition.settings?.ports?.find((p) => p.id === this._id)
  }
  ...
  get type(): string {
    return (
      (this._type === undefined ? this.nodeComponentPortState?.type : this._type) ??
  ...
  update = (state?: IPortStateWithoutIds) => {
    ...
    state.offsetFromNodeCenter !== undefined &&
      this.setOffsetFromNodeCenter(state?.offsetFromNodeCenter)
    state.offsetFromOrigin !== undefined && this.setOffsetFromOrigin(state?.offsetFromOrigin)
  }
  ...
  get data() {
    return this._data === undefined ? this.nodeComponentPortState?.data : this._data
  }
  ...
  get connectedPorts(): PortState[] {
    return this.connectedLinks
      .map((v) => (v.sourceEndpoint.portId === this._id ? v.target?.port : v.source?.port))
...
function getTranslate(item: HTMLElement): Point {
  ...
  if (mat) {
    transArr = [parseFloat(mat[1].split(', ')[4]), parseFloat(mat[1].split(', ')[5])]
  }

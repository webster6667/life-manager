import { Callbacks, SelectionState } from '@easy-diagram/index'
import { makeAutoObservable } from 'mobx'
import { NodeState } from '@easy-diagram/states/nodeState'
import { addPoints, Point } from '@easy-diagram/utils/point'
  ...
  startDragging = (nodeToDrag: NodeState): boolean => {
    ...
    if (nodeToDrag.selected) {
      this._selectionState.unselectItems(
        this._selectionState.selectedItems.filter((i) => i instanceof NodeState && !i.isDragEnabled)
      )
    } else {

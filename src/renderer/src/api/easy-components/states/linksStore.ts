...
} from '@easy-diagram/states/linkPortEndpointState'
import { ILinkState, ILinkStateWithId, LinkState } from '@easy-diagram/states/linkState'
import { createFullPortId, PortState } from '@easy-diagram/states/portState'
import { RootStore } from '@easy-diagram/states/rootStore'
import { guidForcedUniqueness } from '@easy-diagram/utils/guid'
import {
  errorResult,
  errorValueResult,
  isError,
  ...
  successResult,
  successValueResult
} from '@easy-diagram/utils/result'
...
export class LinksStore {
  ...
  import = (newLinks?: ILinkState[]) => {
    ...
    if (newLinks) {
      const results = this._addLinksInternal(newLinks, false)
  ...
  export = (): ILinkState[] => Array.from(this._links).map(([key, value]) => value.export())
  ...
  addLinks = (
    ...
    const results = this._addLinksInternal(links, rewriteIfAlreadyConnected)
  ...
  removeLinks = (linkIds: string[]): SuccessOrErrorResult<ILinkStateWithId, string>[] => {
    const results = this._removeLinks(linkIds)
  ...
  validateLink = (
    ...
    if (!ignoreIfAlreadyConnected && this.areEndpointsConnected(link.source, link.target))
  ...
  getEndpointPortOrError = (endpoint: ILinkPortEndpoint): SuccessOrErrorResult<PortState> => {
    ...
    if (!node) return errorResult(`Node '${endpoint.nodeId}' of link's endpoint does not exist`)
  ...
  getEndpointPort = (endpoint: ILinkPortEndpoint): PortState | undefined => {
    return this._rootStore.nodesStore.getNode(endpoint.nodeId)?.getPort(endpoint.portId)
  }
  ...
  private _addLinksInternal = (
    ...
    const results = links.map((link) => this._addLinkInternal(link, rewriteIfAlreadyConnected))
    return results
  }
  ...
  private _addLinkInternal = (
    ...
    if (rewriteIfAlreadyConnected) {
      ...
      existedLink = this.getLinkForEndpointsIfExists(link.source, link.target)
      if (existedLink) {
  ...
  private _addLinkToNodeLinksCollection = (link: LinkState, nodeId: string) => {
    const links = this._nodesLinksCollection.get(nodeId)
    if (!links) {
  ...
  private _removeLinks = (linkIds: string[]): SuccessOrErrorResult<ILinkStateWithId, string>[] => {
  ...
  private _removeLink = (linkId: string): ILinkStateWithId | undefined => {
    ...
    if (linkToRemove) {
      ...
      this._removeLinkFromNodeLinksCollection(linkToRemove.id, linkToRemove.sourceEndpoint.nodeId)
      this._removeLinkFromNodeLinksCollection(linkToRemove.id, linkToRemove.targetEndpoint.nodeId)
  ...
  private _removeLinkFromNodeLinksCollection = (linkId: string, nodeId: string) => {

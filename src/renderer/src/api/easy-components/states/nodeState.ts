...
import { INodeComponentSettings } from '@easy-diagram/states/nodesSettings'
import {
  IPortExport,
  IPortState,
  IPortStateWithoutIds,
  PortState
} from '@easy-diagram/states/portState'
...
import { deepCopy, Dictionary, isBoolean } from '@easy-diagram/utils/common'
import { addPoints, arePointsEqual, Point } from '@easy-diagram/utils/point'
import { errorResult, SuccessOrErrorResult, successValueResult } from '@easy-diagram/utils/result'
import { generateTransform } from '@easy-diagram/utils/transformation'
import { PropertyChange } from '@easy-diagram/states/callbacks'
...
export class NodeState {
   ...
   */
  moveBy = (vector: Point, ignoreSnapping: boolean = false): Point | undefined => {
  ...
  getPortOrThrowException = (portId: string): PortState => {
    ...
    if (port) return port
    else throw `Port with id '${portId}' does not exist in the node '${this._id}'`
  }

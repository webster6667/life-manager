import {
  ...
  OnNodeTypeChanged
} from '@easy-diagram/states/callbacks'
...
describe('Node state', () => {
  ...
  test('import/export', () => {
    const defaultComponent = store.nodesSettings.visualComponents.getComponent('default')
  ...
  ])('Generate transform string for position %p', (pos, expectedTransformStr) => {
    const node = new NodeState(store, 'test', {
      position: pos
    })
  ...
  describe('onNodeIsSelectionEnabledChanged callback', () => {
    ...
    beforeEach(() => {
      ...
      store.callbacks.import({
        onNodeIsSelectionEnabledChanged: mockNodeIsSelectionEnabledChangedCallback
      })
    })
    ...
    test('Is not called on initialization', () => {
      ...
      expect(mockNodeIsSelectionEnabledChangedCallback.mock.calls.length).toBe(0)
    })
    ...
    test('Is called for new value set by code', () => {
      ...
      expect(node.isSelectionEnabled).toEqual(true)
      expect(mockNodeIsSelectionEnabledChangedCallback.mock.calls.length).toBe(1)
      validateCallbackCall(mockNodeIsSelectionEnabledChangedCallback.mock.calls[0], {
        node: node,
        oldValue: undefined,
        newValue: true
      })

import { ICallbacks, OnNodesAddResult, OnNodesRemoveResult } from '@easy-diagram/states/callbacks'
import { INodeState } from '@easy-diagram/states/nodeState'
import { RootStore } from '@easy-diagram/states/rootStore'
import { createDummyHTMLElement } from '@easy-diagram/utils/__tests__/testUtils'
import React from 'react'
import { isError, isSuccess } from '@easy-diagram/utils/result'
...
describe('Nodes store', () => {
  ...
  describe('Add/remove', () => {
    ...
    const validateOnNodesAddResult = (invocation: number, info: OnNodesAddResult) => {
      ...
      info.addedNodes.forEach((v) => expect(callbackCall[0].addedNodes).toContain(v))

      expect(callbackCall[0].failedToAddNodes.length).toBe(info.failedToAddNodes.length)
      info.failedToAddNodes.forEach((v) => expect(callbackCall[0].failedToAddNodes).toContain(v))

      expect(callbackCall[0].importing).toBe(info.importing)
      expect(callbackCall[1]).toBe(store)
    }

    const validateOnNodesRemoveResult = (invocation: number, info: OnNodesRemoveResult) => {
      const callbackCall = mockOnNodesRemoveResultCallback.mock.calls[invocation]

      expect(callbackCall[0].removedNodes.length).toBe(info.removedNodes.length)
      info.removedNodes.forEach((v) => expect(callbackCall[0].removedNodes).toContain(v))

      expect(callbackCall[0].failedToRemoveNodeIds).toEqual(info.failedToRemoveNodeIds)
      expect(callbackCall[1]).toBe(store)
    }

    test('Import nodes', () => {
      const nodes: INodeState[] = [{ id: 'first', position: [0, 10] }, { position: [100, 110] }]
  ...
  describe('Get nodes bounding box', () => {
    ...
    test('Two nodes with sizes', () => {
      store.nodesStore.import([{ position: [-10, -15] }, { position: [10, 15] }])

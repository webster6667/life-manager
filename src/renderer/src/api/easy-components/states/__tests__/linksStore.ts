import { ICallbacks, OnLinksAddResult, OnLinksRemoveResult } from '@easy-diagram/states/callbacks'
import { ILinkState, LinkState } from '@easy-diagram/states/linkState'
...
describe('Links store', () => {
  ...
  describe('Add/remove', () => {
    ...
    const validateOnLinksAddResult = (invocation: number, info: OnLinksAddResult) => {
      ...
      info.addedLinks.forEach((v) => expect(callbackCall[0].addedLinks).toContain(v))

      expect(callbackCall[0].failedToAddLinks.length).toBe(info.failedToAddLinks.length)
      info.failedToAddLinks.forEach((v) => expect(callbackCall[0].failedToAddLinks).toContain(v))

      expect(callbackCall[0].importing).toBe(info.importing)
      expect(callbackCall[1]).toBe(store)
    }

    const validateOnLinksRemoveResult = (invocation: number, info: OnLinksRemoveResult) => {
      const callbackCall = mockOnLinksRemoveResultCallback.mock.calls[invocation]

      expect(callbackCall[0].removedLinks.length).toBe(info.removedLinks.length)
      info.removedLinks.forEach((v) => expect(callbackCall[0].removedLinks).toContain(v))

      expect(callbackCall[0].failedToRemoveLinkIds).toEqual(info.failedToRemoveLinkIds)
      expect(callbackCall[1]).toBe(store)
    }
    ...
    test('Existed link is not rewritten on adding many links by default', () => {
      ...
      const result = store.linksStore.addLinks([linkToAdd1WithDifference, linkToAdd2])

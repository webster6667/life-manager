...
import { multiplyPoint, subtractPoints } from '@easy-diagram/utils/point'
import { useRootStore } from '@easy-diagram/hooks/useRootStore'
import type { IDragHandlers } from '@easy-diagram/hooks/userInteractions/useDiagramDragHandlers'
import { useDiagramCursor } from '@easy-diagram/hooks/userInteractions/useCursor'
import { ReactDOMAttributes } from '@use-gesture/react/dist/declarations/src/types'

export const usePortUserInteraction = (portState?: PortState): IUsePortUserInteractionResult => {
  const {
  ...
  const handlers = useMemo<IGestureHandlers | {}>(
    () => ({
      ...
      onDragStart: ({ event, xy }) => {
        ...
        portHtmlElement.releasePointerCapture((event as PointerEvent).pointerId)

        const clientRect = portHtmlElement.getBoundingClientRect()
        const pointOnPort = subtractPoints(xy, [clientRect.x, clientRect.y])

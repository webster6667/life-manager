  ...
  const handlers = useMemo<GestureHandlers>(
    () => ({
      ...
      onDrag: ({ pinching, delta, movement, cancel, target, type, ...rest }) => {
        ...
        if (rootStore.dragState.isActive && !nodeState.isDragActive && startedDragRef.current) {
      ...
      onDragStart: ({ event, cancel }) => {
        interactionActiveRef.current = allowNodeInteraction(event) && !rootStore.dragState.isActive
...
export const ENABLE_NODE_USER_INTERACTION_CLASS = 'react_easy_diagram_enable_node_user_interaction'
